...
Page({
  ...
  onLoad: function (options) {
    console.log(options);
    wx.setNavigationBarTitle({ title: "自适应试题推荐" }) // 动态设置导航条标题
    ...
    // this.setData({
    //   shuffleIndex: this.shuffle(count).slice(0, 25) // 生成随机题序 [2,0,3] 并截取num道题
    // })
 ...
 nextSubmit: function () {
  ...
  if (this.data.index < this.data.questionList.length - 1) {
    ...
    wx.request({
      url: 'http://skyhook.cloud:5000/user/record',
      method: "POST",
      data: {  
    ...
    wx.request({
      url: 'http://skyhook.cloud:5000/user/result',
      method: "POST",
      data: {  
        username: app.globalData.username,
    ...
    let logsList = { "date": Date.now(),   "testId": '自适应推荐功能',"score": this.data.totalScore }
